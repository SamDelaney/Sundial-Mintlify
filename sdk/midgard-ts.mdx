...
const config: PhaseAConfig = {
  minFeeA:          44,         // Cardano protocol parameter (lovelace per byte)
...
`midgard-ts` types mirror the on-chain Aiken smart contract data structures from the `onchain/aiken/` source tree. When building off-chain tooling that constructs fraud proofs or state-queue operations, use `midgard-ts` to produce correctly shaped datums and redeemers that match the Plutus V3 validators.
