---
title: "Deploy a Sundial Layer Node: Docker & Nix Guide"
sidebarTitle: "Layer Node"
description: "Run the Sundial-Layer-Node as a block producer on the Sundial L2. Covers Docker Compose setup, split node roles, smoke checks, and Grafana monitoring."
---
...
<Steps>
  ...
  <Step title="Verify the node is running">
    ...
    Then probe the node's API and state queue:

    ```bash
    # Check the state queue endpoint
...
<Steps>
  <Step title="Confirm the node API is reachable">
    ```bash
    curl -fsS http://localhost:3000/stateQueue
    ```

    A JSON response with state-queue data means the node is accepting requests.
  </Step>
...
If you prefer a reproducible Nix-based build instead of Docker, use the flake in the `demo/` directory:
...
<AccordionGroup>
  ...
  <Accordion title="Port conflicts on startup">
    The stack uses ports `3000`, `3001`, `3100`, `3200`, `4317`, `4318`, `5433`, `6379`, `8081`, `9090`, and `9464`. Stop any existing services on those ports, or override them in `.env` using the `_HOST_PORT` variable variants.
  </Accordion>
