---
title: "Faucet API: Claim Testnet sBTC Programmatically"
sidebarTitle: "Faucet API"
description: "POST /faucet/claims sends testnet sBTC to a Cardano preprod address. Accepts a bech32 address and returns a transaction ID. Includes structured error responses."
---

The faucet API lets you claim testnet sBTC programmatically, making it straightforward to fund addresses in automated test suites, CI pipelines, and integration scripts without visiting the web UI. Supply a Cardano preprod address, and the faucet submits an on-chain transaction to fund it. All tokens dispensed by the faucet are testnet-only and carry no real-world value.

<Warning>
  The faucet is a testnet-only service. Claims are subject to per-address and per-IP cooldowns to prevent abuse. Do not rely on the faucet for high-frequency automated funding — pre-fund a set of test wallets at the start of a test run instead.
</Warning>

---

## POST /faucet/claims — Claim Testnet sBTC

Submit a claim request for testnet sBTC. The faucet validates the address, checks cooldown and IP limits, builds a funding transaction, and returns the transaction hash once the transaction is submitted to Cardano preprod.
...
```json
{
  ...
  "ipHash": "<server-generated>"
}
```

<ParamField body="address" type="string" required>
  Bech32 Cardano testnet address to receive the sBTC. Must start with `addr_test1`, must carry a valid payment credential, must not be a script address, and must be a preprod (testnet) network address.
</ParamField>
...
<ParamField body="ipHash" type="string">
  A hashed representation of the requester's IP address, used for per-IP rate limiting. This field is handled server-side when you go through the web frontend proxy. When calling the API directly, include an empty string or omit the field — the server-side proxy at `sundialprotocol.com` populates it automatically.
</ParamField>
...
```typescript
async function claimFaucet(address: string, runId: string): Promise<void> {
  const res = await fetch(
    "https://rpc.testnet.sundialprotocol.com/faucet/claims",
    {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ address, idempotencyKey: runId }),
    }
  );
...
<Steps>
  ...
  <Step title="Call the faucet">
    POST the claim. On a `200` response, store `nextEligibleAt` and proceed with your test.
  </Step>
...
<Info>
  The faucet dispenses testnet sBTC (simulated BTC) on the Cardano preprod network. These tokens are used for testing Sundial L2 transactions and the BTC staking flow. They have no monetary value and cannot be transferred to mainnet.
</Info>
