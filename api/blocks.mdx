---
title: "Block API: Query Sundial L2 Block Data"
sidebarTitle: "Blocks"
description: "GET /block retrieves L2 block data including the block header, MPT root hashes, and transaction list. GET /stateQueue returns the current state queue on Cardano L1."
---
...
<Note>
  Both endpoints are available on full (monolith) nodes. `GET /stateQueue` is a full-node-only endpoint; the related `GET /stateQueue/root-unit-diagnostics` is also served by `api`-role nodes and is useful for health monitoring without the full state-queue fetch.
</Note>
...
Retrieve the list of transaction hashes included in a Sundial L2 block, identified by its 56-character header hash. The node looks up the block in `BlocksTxsDB` and returns the hashes of every transaction committed to that block.
...
<Tabs>
  <Tab title="Success (200)">
    ...
    Each entry in `hashes` is a 64-character hex transaction hash. An empty `hashes` array indicates a block with no transactions (an empty block). Use each hash with `GET /tx` to retrieve the full CBOR of individual transactions.
  </Tab>
...
Each entry in `headers` is a 56-character hex block header hash representing a committed block on the state queue. The list is ordered from oldest to newest as stored in the on-chain linked list.
...
<Accordion title="Merged — incorporated into confirmed ledger state">
  The block has been merged into the Sundial confirmed state. Its transactions are permanently recorded in `BlocksTxsDB` and reachable via `GET /block`.
</Accordion>
